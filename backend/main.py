...
def check_birthdays_and_notify_admin():
    try:
        ...
        # 1. Clean out orphan birthday alerts for employees who are no longer active or present
        active_emp_map = {
            u["id"]: u for u in db_state.get("users", [])
            if u.get("role") == "employee" and u.get("status") == "active"
        }
        active_emp_emails = {
            u.get("email", "").strip().lower() for u in active_emp_map.values() if u.get("email")
        }

        messages = db_state.get("messages", [])
        cleaned_messages = []
        purged = False
        for msg in messages:
            if msg.get("type") == "birthday_alert":
                msg_id = msg.get("id", "")
                parts = msg_id.split("-")
                emp_id_in_alert = "-".join(parts[2:-1]) if len(parts) >= 4 else ""
                msg_body = msg.get("body", "").lower()

                is_active_emp = False
                if emp_id_in_alert and emp_id_in_alert in active_emp_map:
                    is_active_emp = True
                elif any(email in msg_body for email in active_emp_emails if email):
                    is_active_emp = True

                if not is_active_emp:
                    print(f"[Birthday Scan] Purged orphan birthday alert: {msg_id}")
                    purged = True
                    continue
            cleaned_messages.append(msg)

        if purged:
            db_state["messages"] = cleaned_messages
            save_database()

        print(f"[Birthday Scan] Running sweeps. Tomorrow MM-DD: {md_tomorrow}, 1 Week: {md_one_week}")
        
        for user in active_emp_map.values():
            if user.get("dob"):
                try:
...
def seed_database():
    ...
    db_state["holidays"] = [
        ...
        },
        {
            ...
            "name": "Republic Day",
            "type": "National",
            "description": "Indian Republic Day Holiday",
            "createdAt": now_iso
        },
        {
            "id": "hol-2026-08-15",
            ...
            "type": "National",
            "description": "Indian Independence Day Holiday",
            "createdAt": now_iso
        },
        {
            "id": "hol-2026-10-02",
...
def delete_employee(user_id: str):
    ...
    db_state["checklists"] = [c for c in db_state["checklists"] if c.get("employeeId") != user_id]
    db_state["assignedTests"] = [t for t in db_state["assignedTests"] if t.get("employeeId") != user_id]
    db_state["notifications"] = [n for n in db_state["notifications"] if n.get("employeeId") != user_id]
    db_state["taskSubmissions"] = [s for s in db_state.get("taskSubmissions", []) if s.get("employeeId") != user_id]
    db_state["attendance"] = [att for att in db_state.get("attendance", []) if att.get("employeeId") != user_id]
    db_state["leaves"] = [l for l in db_state.get("leaves", []) if l.get("employeeId") != user_id]
    
    # Purge all messages sent by, received by, or referencing deleted user_id or their birthday alerts
    db_state["messages"] = [
        m for m in db_state.get("messages", [])
        if m.get("senderId") != user_id
        and m.get("receiverId") != user_id
        and not m.get("id", "").startswith(f"bday-1day-{user_id}-")
        and not m.get("id", "").startswith(f"bday-7day-{user_id}-")
    ]
    
    if "passwords" in db_state and user_id in db_state["passwords"]:
        del db_state["passwords"][user_id]
...
# ==========================================
# EMPLOYEE TRACKER & HOLIDAY MANAGEMENT MODULE
# ==========================================

@app.post("/api/attendance/checkout")
def record_attendance_checkout(req: Dict[str, Any]):
    emp_id = req.get("employeeId")
    date_val = req.get("date")  # YYYY-MM-DD
    ...
    record = next((r for r in db_state.get("attendance", []) if r.get("employeeId") == emp_id and r.get("date") == date_val), None)
    if not record:
    ...
    log_activity(emp_id, record.get("employeeName", "Employee"), "Attendance Checked Out", f"Recorded check-out for {date_val}.", sync=False)
    save_database(["attendance", "activityLogs"])
    return record


# Holiday Management Endpoints
@app.get("/api/employee-tracker/holidays")
def list_holidays():
    return db_state.get("holidays", [])

@app.post("/api/employee-tracker/holidays", status_code=201)
def create_holiday(req: Dict[str, Any]):
    date_val = req.get("date")  # YYYY-MM-DD
    name = req.get("name")
    h_type = req.get("type", "Company")
    description = req.get("description", "")
    
    if not date_val or not name:
        raise HTTPException(status_code=400, detail="Holiday date and name are required.")
        
    holidays = db_state.get("holidays", [])
    existing = next((h for h in holidays if h.get("date") == date_val), None)
    if existing:
        raise HTTPException(status_code=400, detail=f"A holiday already exists for date {date_val}.")
        
    new_holiday = {
        "id": f"hol-{int(datetime.now().timestamp() * 1000)}-{str(random.randint(100, 999))}",
        "date": date_val,
        "name": name,
        "type": h_type,
        "description": description,
    ...
    if "holidays" not in db_state:
        db_state["holidays"] = []
    db_state["holidays"].append(new_holiday)
    db_state["holidays"].sort(key=lambda h: h["date"])
    
    save_database(["holidays"])
    log_activity("admin-1", "Admin", "Holiday Created", f"Added holiday '{name}' on {date_val}.", sync=False)
    return new_holiday

@app.put("/api/employee-tracker/holidays/{holiday_id}")
def update_holiday(holiday_id: str, req: Dict[str, Any]):
    holidays = db_state.get("holidays", [])
...
@app.delete("/api/employee-tracker/holidays/{holiday_id}")
def delete_holiday(holiday_id: str):
    holidays = db_state.get("holidays", [])
    db_state["holidays"] = [h for h in holidays if h["id"] != holiday_id]
    save_database(["holidays"])
    return {"success": True, "message": "Holiday removed."}


# Evaluation Helper Logic
def _parse_time_to_minutes(iso_str: Optional[str]) -> Optional[int]:
    if not iso_str:
        return None
    try:
        if "T" in iso_str:
            dt = datetime.fromisoformat(iso_str.replace("Z", "+00:00"))
            # Convert UTC to local IST (+5:30) if timezone offset is UTC (+00:00)
...
def _format_time_display(iso_str: Optional[str]) -> str:
    if not iso_str:
        return "--:--"
    m = _parse_time_to_minutes(iso_str)
    if m is None:
        return "--:--"
    h = m // 60
    mins = m % 60
...
def _evaluate_single_day(employee: Dict[str, Any], date_str: str, attendance_records: List[Dict[str, Any]], holidays_map: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    try:
    ...
    weekday = dt_obj.weekday()  # 0=Monday, 4=Friday, 5=Saturday, 6=Sunday
    ...
    if is_weekend or is_holiday:
        return {
            ...
            "dayName": day_name,
            "isWorkingDay": False,
            "dayType": "holiday" if is_holiday else "weekend",
            "holidayName": holiday_info["name"] if holiday_info else None,
            "checkInTime": None,
            "checkOutTime": None,
            "totalWorkingMinutes": 0,
            "totalWorkingFormatted": "0h 00m",
            "expectedWorkingHours": 8,
            "differenceMinutes": 0,
            "differenceFormatted": "0h 00m",
            "lateMinutes": 0,
    ...
    att = next((r for r in attendance_records if r.get("employeeId") == emp_id and r.get("date") == date_str), None)
    
    if not att or att.get("status") == "rejected":
        return {
            "employeeId": emp_id,
            "employeeName": emp_name,
            "date": date_str,
            ...
            "differenceFormatted": "-8h 00m",
            "lateMinutes": 0,
            "earlyCheckoutMinutes": 0,
            "status": "Absent",
            "score": 0,
            "attendanceStatus": att.get("status") if att else None
        }
        
    checkin_iso = att.get("submittedAt")
    checkout_iso = att.get("checkOutAt")
    
    checkin_mins = _parse_time_to_minutes(checkin_iso)
    checkout_mins = _parse_time_to_minutes(checkout_iso)
    
    expected_checkin_mins = 600   # 10:00 AM
    expected_checkout_mins = 1080  # 6:00 PM
    
    late_mins = 0
    if checkin_mins is not None:
        if checkin_mins > expected_checkin_mins:
            late_mins = checkin_mins - expected_checkin_mins
        punctuality_score = max(0.0, 30.0 - 0.5 * late_mins)
    else:
        punctuality_score = 0.0
        
    early_checkout_mins = 0
    if checkout_mins is not None:
        if checkout_mins < expected_checkout_mins:
            early_checkout_mins = expected_checkout_mins - checkout_mins
        checkout_score = max(0.0, 20.0 - 0.5 * early_checkout_mins)
    else:
        checkout_score = 0.0
        
    total_working_mins = 0
    if checkin_mins is not None and checkout_mins is not None:
        total_working_mins = max(0, checkout_mins - checkin_mins)
    elif checkin_mins is not None:
        today_str = datetime.now().strftime("%Y-%m-%d")
        if date_str == today_str:
            now_mins = datetime.now().hour * 60 + datetime.now().minute
            if now_mins > checkin_mins:
                total_working_mins = min(480, now_mins - checkin_mins)
                
    duration_score = min(50.0, (total_working_mins / 480.0) * 50.0)
    
    diff_mins = total_working_mins - 480
    diff_formatted = f"+{_format_minutes(diff_mins)}" if diff_mins >= 0 else f"-{_format_minutes(abs(diff_mins))}"
    
    total_score = int(round(punctuality_score + duration_score + checkout_score))
    total_score = min(100, max(0, total_score))
    
    if not checkout_iso and date_str != datetime.now().strftime("%Y-%m-%d"):
        status_val = "Incomplete"
    elif total_score >= 90 and late_mins == 0 and early_checkout_mins == 0:
        status_val = "Excellent"
    elif late_mins > 0:
        status_val = "Late"
    elif early_checkout_mins > 0:
        status_val = "Early Checkout"
    elif total_working_mins < 480:
        status_val = "Short Hours"
    else:
        status_val = "On Time"
    ...
    return {
        ...
        "isWorkingDay": True,
        "dayType": "working",
        "checkInTime": _format_time_display(checkin_iso),
        "checkOutTime": _format_time_display(checkout_iso),
        "totalWorkingMinutes": total_working_mins,
        "totalWorkingFormatted": _format_minutes(total_working_mins),
        "expectedWorkingHours": 8,
        "differenceMinutes": diff_mins,
        "differenceFormatted": diff_formatted,
        "lateMinutes": late_mins,
        "earlyCheckoutMinutes": early_checkout_mins,
        "status": status_val,
        "score": total_score,
        "attendanceStatus": att.get("status")
    }

def _get_active_employees() -> List[Dict[str, Any]]:
    return [u for u in db_state.get("users", []) if u.get("role") == "employee" and u.get("status") == "active"]

def _get_holidays_map() -> Dict[str, Dict[str, Any]]:
    return {h["date"]: h for h in db_state.get("holidays", [])}
...
@app.get("/api/employee-tracker/summary")
def get_tracker_summary(date: Optional[str] = None):
    date_val = date or datetime.now().strftime("%Y-%m-%d")
    employees = _get_active_employees()
    holidays_map = _get_holidays_map()
    attendance_records = db_state.get("attendance", [])
    
    dt_obj = datetime.strptime(date_val, "%Y-%m-%d")
    is_weekend = (dt_obj.weekday() == 5 or dt_obj.weekday() == 6)
    is_holiday = date_val in holidays_map
    is_working_today = (not is_weekend) and (not is_holiday)
    
    daily_evals = [_evaluate_single_day(emp, date_val, attendance_records, holidays_map) for emp in employees]
    
    working_today = sum(1 for e in daily_evals if e["isWorkingDay"] and e["status"] in ["Excellent", "On Time", "Late", "Early Checkout", "Short Hours", "Incomplete"])
    late_today = sum(1 for e in daily_evals if e["isWorkingDay"] and e["lateMinutes"] > 0)
    absent_today = sum(1 for e in daily_evals if e["isWorkingDay"] and e["status"] == "Absent")
    
    working_evals = [e for e in daily_evals if e["isWorkingDay"]]
    avg_score = int(round(sum(e["score"] for e in working_evals) / len(working_evals))) if working_evals else 0
    avg_minutes = int(round(sum(e["totalWorkingMinutes"] for e in working_evals) / len(working_evals))) if working_evals else 0
    
    return {
        "date": date_val,
        "isWorkingDayToday": is_working_today,
        "dayNote": ("Non-Working Day (Weekend)" if is_weekend else f"Non-Working Day ({holidays_map[date_val]['name']})") if not is_working_today else "Official Working Day",
        "totalEmployees": len(employees),
        "workingToday": working_today,
        "lateToday": late_today,
        "absentToday": absent_today,
        "avgWorkingHoursFormatted": _format_minutes(avg_minutes),
        "avgScore": avg_score
    }


@app.get("/api/employee-tracker/daily")
def get_daily_tracker(date: Optional[str] = None, employeeId: Optional[str] = None):
    date_val = date or datetime.now().strftime("%Y-%m-%d")
    employees = _get_active_employees()
    if employeeId:
        employees = [e for e in employees if e["id"] == employeeId]
        
    holidays_map = _get_holidays_map()
    attendance_records = db_state.get("attendance", [])
    
    results = [_evaluate_single_day(emp, date_val, attendance_records, holidays_map) for emp in employees]
    return results


@app.get("/api/employee-tracker/weekly")
def get_weekly_tracker(startDate: Optional[str] = None, employeeId: Optional[str] = None):
    today = datetime.now()
    if not startDate:
    ...
    for emp in employees:
        daily_evals = [_evaluate_single_day(emp, d, attendance_records, holidays_map) for d in dates]
        working_evals = [e for e in daily_evals if e["isWorkingDay"]]
        
        evaluated_days = len(working_evals)
        present_days = sum(1 for e in working_evals if e["status"] in ["Excellent", "On Time", "Late", "Early Checkout", "Short Hours", "Incomplete"])
        absent_days = sum(1 for e in working_evals if e["status"] == "Absent")
        late_days = sum(1 for e in working_evals if e["lateMinutes"] > 0)
        early_checkout_days = sum(1 for e in working_evals if e["earlyCheckoutMinutes"] > 0)
        
        total_mins = sum(e["totalWorkingMinutes"] for e in working_evals)
        avg_daily_mins = int(round(total_mins / evaluated_days)) if evaluated_days > 0 else 0
        avg_score = int(round(sum(e["score"] for e in working_evals) / evaluated_days)) if evaluated_days > 0 else 0
        
        weekly_summaries.append({
            "employeeId": emp["id"],
            "employeeName": emp["name"],
            "weekLabel": f"{dates[0]} to {dates[6]}",
            "startDate": dates[0],
            "endDate": dates[6],
            "totalDays": 7,
            "weekendDays": 2,
            "holidayDays": sum(1 for e in daily_evals if e["dayType"] == "holiday"),
            "expectedWorkingDays": evaluated_days,
            "evaluatedDays": evaluated_days,
            "presentDays": present_days,
            "absentDays": absent_days,
            "lateDays": late_days,
            "earlyCheckoutDays": early_checkout_days,
            "totalWorkingMinutes": total_mins,
            "totalWorkingFormatted": _format_minutes(total_mins),
            "avgDailyMinutes": avg_daily_mins,
            "avgDailyWorkingFormatted": _format_minutes(avg_daily_mins),
            "avgCheckInTime": "10:00 AM",
            "avgCheckOutTime": "06:00 PM",
            "weeklyScore": avg_score,
            "status": _classify_score(avg_score)
        })
        
    return weekly_summaries


@app.get("/api/employee-tracker/monthly")
def get_monthly_tracker(yearMonth: Optional[str] = None, employeeId: Optional[str] = None):
    ym_val = yearMonth or datetime.now().strftime("%Y-%m")
    year, month = map(int, ym_val.split("-"))
    import calendar
    num_days = calendar.monthrange(year, month)[1]
    
    dates = [f"{year:04d}-{month:02d}-{d:02d}" for d in range(1, num_days + 1)]
    employees = _get_active_employees()
    if employeeId:
        employees = [e for e in employees if e["id"] == employeeId]
    ...
    for emp in employees:
        ...
        weekend_days = sum(1 for e in daily_evals if e["dayType"] == "weekend")
        holiday_days = sum(1 for e in daily_evals if e["dayType"] == "holiday")
        evaluated_days = len(working_evals)
        present_days = sum(1 for e in working_evals if e["status"] in ["Excellent", "On Time", "Late", "Early Checkout", "Short Hours", "Incomplete"])
        absent_days = sum(1 for e in working_evals if e["status"] == "Absent")
        late_days = sum(1 for e in working_evals if e["lateMinutes"] > 0)
...
@app.get("/api/employee-tracker/yearly")
def get_yearly_tracker(year: Optional[str] = None, employeeId: Optional[str] = None):
    y_str = year or str(datetime.now().year)
    y_int = int(y_str)
    ...
    for emp in employees:
        ...
        total_admin_holidays_year = 0
        total_evaluated_days_year = 0
        total_present_year = 0
        total_absent_year = 0
        total_late_year = 0
        total_early_checkout_year = 0
        total_mins_year = 0
        sum_scores_year = 0
        
        for m in range(1, 13):
            ym = f"{y_int:04d}-{m:02d}"
            m_name = datetime(y_int, m, 1).strftime("%B")
            num_days = calendar.monthrange(y_int, m)[1]
            dates = [f"{y_int:04d}-{m:02d}-{d:02d}" for d in range(1, num_days + 1)]
            
            daily_evals = [_evaluate_single_day(emp, d, attendance_records, holidays_map) for d in dates]
            working_evals = [e for e in daily_evals if e["isWorkingDay"]]
            
            w_days = len(working_evals)
            p_days = sum(1 for e in working_evals if e["status"] in ["Excellent", "On Time", "Late", "Early Checkout", "Short Hours", "Incomplete"])
            a_days = sum(1 for e in working_evals if e["status"] == "Absent")
            l_days = sum(1 for e in working_evals if e["lateMinutes"] > 0)
            
            m_mins = sum(e["totalWorkingMinutes"] for e in working_evals)
            m_avg_mins = int(round(m_mins / w_days)) if w_days > 0 else 0
            m_score = int(round(sum(e["score"] for e in working_evals) / w_days)) if w_days > 0 else 0
            ...
            month_rows.append({
                ...
                "absent": a_days,
                "late": l_days,
                "avgHoursFormatted": _format_minutes(m_avg_mins),
                "monthlyScore": m_score
            })
            
            total_working_days_year += w_days
            total_admin_holidays_year += sum(1 for e in daily_evals if e["dayType"] == "holiday")
            total_evaluated_days_year += w_days
            total_present_year += p_days
            total_absent_year += a_days
            total_late_year += l_days
            total_early_checkout_year += sum(1 for e in working_evals if e["earlyCheckoutMinutes"] > 0)
            total_mins_year += m_mins
            sum_scores_year += sum(e["score"] for e in working_evals)
            
        avg_yearly_score = int(round(sum_scores_year / total_evaluated_days_year)) if total_evaluated_days_year > 0 else 0
        avg_daily_mins_year = int(round(total_mins_year / total_evaluated_days_year)) if total_evaluated_days_year > 0 else 0
        
        yearly_summaries.append({
            "employeeId": emp["id"],
            "employeeName": emp["name"],
            "year": y_str,
            "totalWorkingDays": total_working_days_year,
            "totalAdminHolidays": total_admin_holidays_year,
            "totalEvaluatedDays": total_evaluated_days_year,
            "totalPresentDays": total_present_year,
            "totalAbsentDays": total_absent_year,
            "totalLateDays": total_late_year,
            "totalEarlyCheckouts": total_early_checkout_year,
            "totalWorkingMinutes": total_mins_year,
            "totalWorkingFormatted": _format_minutes(total_mins_year),
            "avgDailyMinutes": avg_daily_mins_year,
            "avgDailyWorkingFormatted": _format_minutes(avg_daily_mins_year),
            "yearlyScore": avg_yearly_score,
            "status": _classify_score(avg_yearly_score),
...
@app.get("/api/employee-tracker/employee/{employee_id}")
def get_employee_tracker_details(employee_id: str):
    employees = _get_active_employees()
    ...
    total_mins = sum(e["totalWorkingMinutes"] for e in working_evals)
    avg_score = int(round(sum(e["score"] for e in working_evals) / evaluated_days)) if evaluated_days > 0 else 0
    avg_mins = int(round(total_mins / evaluated_days)) if evaluated_days > 0 else 0
    ...
    return {
        ...
        "summary": {
            ...
            "absentDays": absent_days,
            "lateDays": late_days,
            "avgWorkingHoursFormatted": _format_minutes(avg_mins),
            "overallScore": avg_score,
            "status": _classify_score(avg_score)
        },
        "dailyEvaluations": daily_evals
    }
