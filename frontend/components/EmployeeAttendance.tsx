...
export default function EmployeeAttendance({ currentUser, onRefreshAll }: EmployeeAttendanceProps) {
  ...
  // Handle Daily Check-out request
  const handleCheckOut = async () => {
    ...
    try {
      ...
      setCheckInError("Connection failure occurred during check-out.");
    } finally {
                    ...
                    <div className="flex flex-col items-center space-y-1">
                      ...
                      {!todayRecord.checkOutAt ? (
                        <button
                          ...
                          className="mt-2 px-5 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-xs rounded-lg shadow transition-all cursor-pointer uppercase flex items-center gap-1.5"
                        >
                          <Clock className="h-3.5 w-3.5" />
                          {checkingOut ? "Recording..." : "Record Check-Out Time"}
                        </button>
                      ) : (
                        <span className="text-xs font-semibold text-indigo-600 font-mono mt-1">
                          Checked Out: {new Date(todayRecord.checkOutAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      )}
